...
class TensorboardWriterHook(HookBase):
    ...
    def write(self):
        for key, (iter, value) in self.trainer.metric_storage.values_maybe_smooth:
...
class TerminalWriterHook(HookBase):
    ...
    def __init__(self, period: int = 20) -> None:
        ...
        """
        self._period = period
        self._max_iters = self.trainer.max_iters
    ...
    def _get_eta(self) -> str:
        ...
        if avg_iter_time is not None:
            ...
            eta_seconds = avg_iter_time * (self._max_iters - iter - 1)
            ...
            if self._last_write is not None:
                ...
                eta_seconds = estimated_iter_time * (self._max_iters - iter - 1)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
            self._last_write = (iter, time.perf_counter())
            return eta_string

    def write(self) -> None:
        data_time = self._storage.values_maybe_smooth["data_time"]
        iter_time = self._storage.global_avg.get("iter_time", None)
        lr = self._storage.values_maybe_smooth["lr"]
        ...
        loss_strings = [
            f"{key}: {value:.4g}"
            for key, value in self._storage.values_maybe_smooth.items()
            if "loss" in key
        ]
